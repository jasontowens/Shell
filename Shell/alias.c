...
void
add_alias (char* name, char* value, int globbed)
{
  BUCKET_CONTENTS *elt;
  alias_t *temp;
  int n;
  ...
  if (temp)
    ...
    {
      temp = (alias_t *)malloc (sizeof (alias_t));
...
int checkBuiltIns(char* value){
	if(strcmp(value,"alias") == 0){
		return 1;
	}
	else if(strcmp(value,"unalias") == 0){
		return 1;
	}
	return 0;
}
...
int addMyAlias(char* name, char* value, int globbed){
	if(strcmp(name,value) == 0){
		printf("You can't alias the same thing\n");
		return 0;
	}
	if(checkBuiltIns(value)){
		printf("You can't alias a \"alias\" or \"unalias\" \n");
		return 0;
	}
	while(find_alias(value)!=NULL){
			if(strcmp(get_alias_value(value),name) == 0){
			printf("infinite Alias\n");
			return 0;
			}
			value = get_alias_value(value);
		}
	add_alias(name,value,globbed);
	++numAlias;
	return 1;
}
char* fixWord(char* word){
	char* returnword;
	returnword = get_alias_value(word);
	printf("argument:%s\n",returnword);
	if(returnword == NULL){
		printf("There isnt anything aliased to that word\n");
		return returnword;
	}else{
		while(get_alias_value(returnword) != NULL){
			returnword = get_alias_value(returnword);
		}
	}
	return returnword;
}
void fixAlias(COMMAND* command){
	...
	int l;
	for(l = 0; l != j; ++l){
		command->arguments->args[l] = myargs[l];
		//printf ("daargbitch%s\n",command->arguments->args[l]);
	}
	command->numArgs = l;
     ...
     sh_alias_map_func_t *function;
{
  ...
  list = (alias_t **)malloc ((i + 1) * sizeof (alias_t *));
